...
class DataCollection<T extends { name: string }> {
    ...
    collate<U> (targetData: U[], itemProp: string, targetProp: string) {
        let results = [];
...
class SearchableCollection<T extends { name: string }> extends DataCollection<Employee>{
...
let emploeeData = new SearchableCollection(employees);
emploeeData.find("Sales").forEach(e => console.log(`Employee: ${e.name}, ${e.role}`));